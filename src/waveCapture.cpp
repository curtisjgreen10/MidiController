...
// Define: getWaveOutNumDevices()
WORD waveCapture::getWaveOutNumDevices()
{
...
// Define: getWaveOutDevName(DWORD deviceID)
LPSTR waveCapture::getWaveOutDevName(const WORD uDev)
{
	if ( waveOutGetDevCaps(uDev, &woutcaps, sizeof(woutcaps)) == MMSYSERR_NOERROR )
	{
		return woutcaps.szPname;
	} else {
		return NULL;
	}
}
...
DWORD waveCapture::_start(const WORD uDevice, const DWORD dwBufferLength, const DWORD dwNumBuffers)
{
	...
	// WaveInOpen
	if(waveInOpen(&hwi,uDevice,(LPWAVEFORMATEX)&wf,(DWORD_PTR)hevent,0,CALLBACK_EVENT) != MMSYSERR_NOERROR)
...
bool waveCapture::closeWAVEFile()
{
	...
	//had to chop off some of the total_buff size for play back to work
	mmckinfoData.cksize = total_buff - 100;
